...
export function EditLectureForm({ lecture }: EditLectureFormProps) {
  ...
  const [uploadedFile, setUploadedFile] = useState<UploadedFile | null>(() => {
    if (!lecture.download_path) return null;
    const name = lecture.download_path.split("/").pop() || "File";
    return { name, path: lecture.download_path };
  });

  const [useExternalLink, setUseExternalLink] = useState<boolean>(() => !!lecture.download_url);
  ...
  const {
    ...
    defaultValues: {
      ...
      download_label: lecture.download_label || "Download Resources",
  ...
  const onSubmit = async (data: LectureFormData) => {
    ...
    try {
      const payload = {
        ...
        // ✅ choose: external link OR uploaded file
      ...
      const { error } = await supabase.from("lectures").update(payload).eq("id", lecture.id);
      if (error) throw error;
      ...
      toast({
        variant: "destructive",
        title: "Update failed",
        description: err?.message ?? "Failed to update lecture. Please try again.",
      });
    } finally {
      setIsLoading(false);
    }
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1">
          ...
          <p className="text-muted-foreground">
            Update lecture details, YouTube link, and downloadable resources.
          </p>
        </div>
